...
public class JobResty extends SpaceResty {
	...
	public Payload putJob(String name, SpaceJob job) {
		Server.context().credentials().checkAtLeastSuperAdmin();
		IndexResponse response = SettingsResty.get().saveAsObject(job);
	...
	public Payload deleteJob(String name) {
		Server.context().credentials().checkAtLeastSuperAdmin();
		boolean deleted = SettingsResty.get()//
	...
	public void manageTasks() {
		try {
			...
			for (SpaceTask task : tasks) {
				...
				Runnable executeCommand = () -> JobResty.this.execute(task);
	...
	private static JobResty singleton = new JobResty();

	public static JobResty get() {
		return singleton;
	}
