...
public class StripeResty extends SpaceResty {
	...
	private void updateStripeCustomerId(Credentials credentials, String stripeCustomerId) {
		...
		CredentialsResty.get().update(credentials);
	}
	...
	private StripeSettings stripeSettings() {
		return SettingsResty.get().getAsObject(StripeSettings.class);
	}
	...
	private static StripeResty singleton = new StripeResty();

	public static StripeResty get() {
		return singleton;
	}

	private StripeResty() {
		SettingsResty.get().registerSettings(StripeSettings.class);
	}
}
