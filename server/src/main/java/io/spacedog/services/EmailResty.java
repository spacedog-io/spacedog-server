...
public class EmailResty extends SpaceResty {
	...
	public Payload postEmail(EmailRequest email, Context context) {
		...
		if (email instanceof EmailBasicRequest) {
			EmailSettings settings = SettingsResty.get().getAsObject(EmailSettings.class);
	...
	private static EmailResty singleton = new EmailResty();

	public static EmailResty get() {
		return singleton;
	}

	private EmailResty() {
		SettingsResty.get().registerSettings(EmailSettings.class);
	}
}
