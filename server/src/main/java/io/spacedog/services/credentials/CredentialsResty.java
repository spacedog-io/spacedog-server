...
import io.spacedog.services.data.SearchResults;
...
public class CredentialsResty extends SpaceResty {
	...
	public Payload postSendPasswordResetEmail(String body, Context context) {
		...
		EmailTemplate template = EmailResty.get()//
		...
		EmailResty.get().email(template, mailContext);
	...
	public Optional<Credentials> getById(String id, boolean throwNotFound) {
	...
	private static CredentialsResty singleton = new CredentialsResty();

	public static CredentialsResty get() {
		return singleton;
	}

	private CredentialsResty() {
		SettingsResty.get().registerSettings(CredentialsSettings.class);
	}
}
