...
public class FileResty extends SpaceResty {
	...
	private Payload saveBucketSettings(String bucket, Context context) {
		InternalFileSettings fileSettings = SettingsResty.get()//
		...
		SettingsResty.get().saveAsObject(fileSettings);
	...
	public FileBucketSettings bucketSettings(String bucket) {
		FileBucketSettings bucketSettings = SettingsResty.get()//
	...
	private static FileResty singleton = new FileResty();

	public static FileResty get() {
		return singleton;
	}

	private FileResty() {
		SettingsResty.get().registerSettings(InternalFileSettings.class);
		initFileStore();
	}
}
