...
public class PushResty extends SpaceResty {
	...
	public Payload postPushRequest(String body, Context context) {
		...
		PushSettings settings = SettingsResty.get().getAsObject(PushSettings.class);
	...
	public Payload upsertInstallation(Optional<String> id, String body, Context context) {
		...
		return id.isPresent() //
				? DataResty.get().doPut(installation, false, context) //
				: DataResty.get().doPost(installation, context);
	}
	...
	private static PushResty singleton = new PushResty();

	public static PushResty get() {
		return singleton;
	}

	private PushResty() {
		SettingsResty.get().registerSettings(PushSettings.class);
	}
}
