...
public class DataResourceTest2 extends AbstractTest {
	...
	public void shouldCreateSearchUpdateAndDeleteSales() throws Exception {
		...
		JsonObject sale = Json.builder().add("number", "1234567890").stObj("where").add("lat", -55.6765)
				.add("lon", -54.6765).end().add("when", "2015-01-09T15:37:00.123Z").add("online", false)
				.add("deliveryDate", "2015-09-09").add("deliveryTime", "15:30:00").stArr("items").addObj()
				.add("ref", "JDM").add("description", "2 rooms appartment in the heart of montmartre")
				.add("quantity", 8)
				// .add("price", "EUR230")
				.add("type", "appartment").end().addObj().add("ref", "LOUVRE")
				.add("description", "Louvre museum 2 days visit with a personal guide") //
				.add("quantity", 2) //
				// .add("price", "EUR54.25") //
				.add("type", "visit").end() //
				.build();
		...
		assertEquals("2 rooms appartment in the heart of montmartre", Json.get(res1, "items.0.description").asString());
		...
		assertEquals("Louvre museum 2 days visit with a personal guide",
				Json.get(res1, "items.1.description").asString());
		...
		RequestBodyEntity req1a = preparePost("/v1/user/", AdminResourceTest.testClientKey()).body(Json.builder()
